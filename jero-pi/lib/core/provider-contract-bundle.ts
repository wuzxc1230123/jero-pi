// jero-pi provider contract bundle verifier (gentle-pi#311 P1/P2).
//
...
// Exactly the provider roles jero-pi supports, in the manifest's strict order.
...
// capability means this jero-pi build cannot satisfy the role: fail closed.
...
function parseManifest(payload: Buffer): ParsedManifest {
	...
	if (major !== SUPPORTED_PROVIDER_CONTRACT_MAJOR) {
		invalid(`unsupported provider contract major ${major}; jero-pi supports major ${SUPPORTED_PROVIDER_CONTRACT_MAJOR} only`);
	}
