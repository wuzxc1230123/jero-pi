...
import { projectJeroConsentEnvelopeV1, projectJeroStatusToWireV1 } from "./authority/wire.ts";
...
// attempt pair, and since P4d-e the START pair - direct starts plus the
// consent question/answer ceremony (the envelope is minted wire-side in
// authority/wire.ts; the answer re-freezes the exact candidate the minted
// invocation binds). Still on the fail-closed P1 stub: the FINALIZE/VALIDATE
// capture quartet and the maintenance quartet's interactive approval flow -
// every served method is typed-union in, wire record out, with authority
// refusals surfacing as errors carrying the typed code.
...
export function createJeroAuthorityReviewCli(): Pick<NativeReviewCli, "sddStatus" | "sddContinue" | "targetStatus" | "reviewMode" | "assess" | "sddAttemptAcquire" | "sddAttemptSettle" | "start" | "answerConsent"> {
	return {
