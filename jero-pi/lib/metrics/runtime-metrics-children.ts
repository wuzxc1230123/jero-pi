...
import { parseAgentDefinition, type AgentDefinition, type ModelRef } from "../agents/agents-config.ts";
import type { ChildObservationSnapshot } from "../agents/agents-runner.ts";
import { FINISHED_STATUSES, type TaskStatus } from "../agents/agents-protocol.ts";
...
export const CHILD_METRICS_EVENT = "jero:runtime-metrics:child/v1";
// Local revocation notification invalidates active observations. Contains only
// the local session join, never policy output.
export const CHILD_METRICS_REVOKED = "jero:runtime-metrics:revoked/v1";
