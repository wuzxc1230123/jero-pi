...
if (process.env.GENTLE_PI_SKIP_GENTLE_AI_INSTALL === "1") {
	...
	try {
		...
		console.error(`jero-pi could not install its package-local Gentle AI v${INSTALLER_VERSION} binary: ${error instanceof Error ? error.message : String(error)}`);
		process.exitCode = 1;
	}
}
...
if (!process.exitCode) {
	try {
		...
		if (result.changed) console.log("jero-pi enabled fullscreen in global Pi settings; /settings can switch back to regular.");
	} catch (error) {
		console.error(`jero-pi could not enable fullscreen: ${error instanceof Error ? error.message : String(error)}`);
		process.exitCode = 1;
	}
}
