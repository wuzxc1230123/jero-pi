...
export async function installTuiModeSetting(options = {}) {
	...
	// jero-pi owned install locations. The git mirror path is provisional until
	// the canonical repository URL is finalized during the P5 identity pass.
	const installations = [
		{
		...
		{
			packageRoot: join(home, "git", "github.com", "jero-pi", "jero-pi"),
			paths: [home, join(home, "git"), join(home, "git", "github.com"), join(home, "git", "github.com", "jero-pi"), join(home, "git", "github.com", "jero-pi", "jero-pi")],
		},
	];
...
// postinstall entry: the tuiMode write is an optional convenience and must never
// fail the install itself.
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	try {
		const result = await installTuiModeSetting();
		if (result.changed) process.stdout.write("jero-pi: enabled Pi fullscreen tuiMode\n");
	} catch (error) {
		process.stdout.write(`jero-pi: skipped tuiMode setting (${error instanceof Error ? error.message : String(error)})\n`);
	}
}
