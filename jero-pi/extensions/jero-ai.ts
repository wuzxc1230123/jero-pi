		...
		pi.on("context", (event, ctx) => {
			...
			if (disciplineBootstrapPending) {
				...
				if (specIndexEnabled(permissionEnvironment)) {
					try {
						// specIndexTextFor 先查 marker 再扫盘：注入窗口内的后续请求
						// 在索引已在上下文中时零磁盘开销。
						const base = messages ?? event.messages;
						messages = applySpecIndex(base, specIndexTextFor(base, ctx.cwd)) ?? messages;
					} catch { /* 索引读取失败绝不阻塞请求。 */ }
				}
			}
			if (sddBreadcrumbEnabled(permissionEnvironment)) {
				try {
					messages = applySddBreadcrumb(messages ?? event.messages, renderSddBreadcrumb(cachedResolveSddStatus(ctx.cwd, resolveSddStatus))) ?? messages;
