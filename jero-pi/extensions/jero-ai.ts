...
import {
	...
	type ThinkingLevel,
} from "../lib/core/model-routing-authority.ts";
import {
...
import { measureAgentsViewLayout, type AgentsViewLayout } from "../lib/agents/agents-view-layout.ts";
...
import { renderGentleAiLifecycleCall, renderGentleAiResult, type GentleAiRenderContext } from "../lib/core/gentle-ai-renderer.ts";
import { sanitizeTerminalText, stripAnsi } from "../lib/core/terminal-theme.ts";
import { CandidateViewError, CandidateViewRegistry, injectReviewCandidateView, readCandidateContextManifestPage, resolveCanonicalCandidateBase, type CandidateView } from "../lib/review/review-candidate-view.ts";
import {
...
} from "../lib/review/review-risk-assessment.ts";
import {
...
import { acquireChildStandingReviewPermissionClient, type ChildStandingReviewPermissionClient } from "../lib/review/review-session-standing-permission-ipc.ts";
import { isPiConsentV3, presentReviewConsentUi } from "../lib/review/review-consent-ui.ts";
import {
...
const BACKGROUND_SUBAGENTS_SCHEMA = "jero-pi.background-subagents/v1";
...
/**
 * Strict decode of {"schema":"jero-pi.background-subagents/v1","policy":"on"|"off"}.
...
// verification plan (`lib/review/review-risk-assessment.ts`), for the `gentle_review`
...
interface ReviewAssessmentPlanDetails {
	schema: "jero-pi.review-assessment-plan/v1";
...
const MODEL_EXPORT_KIND = "jero-pi.agent_model_routing";
...
// relay in lib/review/review-host-relay.ts.
...
//
// Field report (2026-08-16, jero-pi 402f9f77): hydration must run from
...
export interface GentleAiRuntimeDependencies {
	...
	// Test-only seam for the foreground `/jero:telemetry` slash command's
...
function createGentleAiExtensionForTesting(
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		pi.sendMessage(
			{
				customType: "jero-pi.review-preflight",
	...
	pi.registerCommand("jero:dev-binary", {
	...
	pi.registerCommand("jero:doctor", {
		...
		handler: async (_args, ctx) => {
			...
			const lines = [
				...
				...(devBinary.state === "invalid" ? [`fail: ${devBinary.line}`, "remedy: fix the dev binary override or clear it with /jero:dev-binary off (or unset GENTLE_PI_GENTLE_AI_DEV_BINARY)"] : []),
			];
	...
	pi.registerCommand("jero:telemetry", {
		...
		handler: async (args, ctx) => {
			...
			if (subAction !== "status" && subAction !== "enable" && subAction !== "disable" && subAction !== "preview") {
				ctx.ui.notify(`Unknown /jero:telemetry sub-action "${subAction}". Use status, enable, disable, or preview.`, "warning");
				return;
			}
