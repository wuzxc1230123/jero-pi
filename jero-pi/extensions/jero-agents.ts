...
import { extractParentConfirmedSddPreflightContext, getPackageAssetOwner, isParentConfirmedSddPreflightContext, SHIPPED_SDD_AGENT_NAMES } from "../lib/sdd/sdd-preflight.ts";
import { NativeReviewCliV216, NativeReviewCliError, createNodeExecFileAdapter, decodeNativeSddStatusV2, type NativeReviewCli, type NativeSddAcquireRequest, type NativeSddSettleRequest } from "../lib/native/native-review-cli.ts";
import { spawn } from "node:child_process";
import { recordReviewMutation } from "../lib/review/review-reminder-receipt.ts";
import { SessionWorktreeRegistry, resolveSessionWorktree, type WorktreeResolver } from "../lib/core/session-worktree-registry.ts";
...
import { sidebarPart } from "../lib/shell/shell-sidebar.ts";
import { invalidateSidebar } from "../lib/shell/shell-sidebar-layout.ts";
import { createCompletionQueue } from "../lib/agents/agents-completion-delivery.ts";
import { AGENT_MODE, discoverAgents, parseAgentDefinition, loadAgentsConfig, resolveAgentProfile, type AgentDefinition, type AgentMode } from "../lib/agents/agents-config.ts";
import { isFinished, TASK_STATUS, TaskStore, type AskRequest, type TaskRecord } from "../lib/agents/agents-protocol.ts";
import { AgentRunner, piCommand, abortReasonText, plannedCommands, type RemediationPlan, type RemediationScope, REMEDIATION_PLAN_ENV, parseRemediationPlan, remediationEvidence, type AskAnswer, type RunnerDeps, type SddChangeSelection, type TaskRequest, type RemediationTerminalFacts } from "../lib/agents/agents-runner.ts";
import { ChildMessenger, type IpcEndpoint } from "../lib/agents/agents-messaging.ts";
import { hasReviewSessionPermission, resolveCanonicalGitRepositoryIdentitySync, type ReviewSessionManager } from "../lib/review/review-session-standing-permission.ts";
import { historyDir, remediationUnresolved, loadHistory, loadStoredTask, pruneHistory, saveTask } from "../lib/agents/agents-history.ts";
import { sessionToMarkdown } from "../lib/agents/agents-transcript.ts";
import { AgentsView } from "../lib/agents/agents-view.ts";
import { PresencePublisher } from "../lib/core/orchestrator-presence.ts";
import { createNativeFullscreenInteraction } from "../lib/native/native-fullscreen-interaction.ts";
import { AGENTS_GLYPH, renderAgentsCard, widgetExpiryMs, widgetRows } from "../lib/agents/agents-widget.ts";
import { CARD_TONE, renderCard } from "../lib/shell/shell-card.ts";
import { openInExternalEditor } from "./jero-shell.ts";
import { resolveGentlePiAgentHome } from "../lib/agents/agent-home.ts";
import { assertResearchCheckpoint, parseResearchPersistence, RESEARCH_PERSISTENCE_ENTRY, canonicalArtifactPath, researchAgent, renderResearchCapabilities, RESEARCH_CHILD_TOOLS_ENV, RESEARCH_SELECTION_ENV, RESEARCH_ARTIFACT_ENV, parseResearchArtifactIntent, researchArtifactCall, researchArtifactReadback, type ResearchArtifactIntent, type ResearchWriteIdentity } from "../lib/sdd/sdd-research-capabilities.ts";
import { CHILD_METRICS_EVENT, CHILD_METRICS_REVOKED, childEvent, launchSelection, type LaunchSelection } from "../lib/metrics/runtime-metrics-children.ts";
import { runtimeMetricsEnvAllows, type RuntimeMetricsPolicyDeps } from "../lib/metrics/runtime-metrics-policy.ts";
