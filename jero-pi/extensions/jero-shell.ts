...
import { renderShellBar, renderShellSidebarBar, shellEnabled, type ShellBarModel, type ShellBarTheme } from "../lib/shell/shell-bar.ts";
import { CHANGE_STATUS, WorktreeChangesTracker, renderChangesWidget, type ChangedFile, type ChangesModel, type GitRunner, type LineCounter, type WorktreeChanges } from "../lib/shell/shell-changes.ts";
import { WorktreeChangesView } from "../lib/shell/shell-changes-view.ts";
import { SessionWorktreeRegistry, SESSION_WORKTREE_CHANGED, resolveSessionWorktree, toolWorktreePath, worktreeGitEnvironment, type WorktreeResolver } from "../lib/core/session-worktree-registry.ts";
import { CARD_TONE, renderCard, type Card, type CardTheme } from "../lib/shell/shell-card.ts";
import { GentleAiDevBinaryOverrideError, resolveGentleAiDevBinaryOverride } from "../lib/core/gentle-ai-binary.ts";
import { framePromptLines, PROMPT_HINT, PROMPT_STATE, withPromptHint, type PromptState } from "../lib/shell/shell-prompt.ts";
import { accountIdFromToken, CODEX_PROVIDER, CODEX_USAGE_URL, parseCodexUsage, parseUsageHeaders, UsageStore, type ProviderUsage } from "../lib/shell/shell-usage.ts";
...
// Gentle Shell: the visual layer jero-pi puts on top of pi. It installs the
...
const REVIEW_PREFLIGHT_TYPE = "jero-pi.review-preflight";
