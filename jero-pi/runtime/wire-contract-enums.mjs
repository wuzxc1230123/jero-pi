...
export const START_ACTIONS = ["created", "resumed", "closed", "blocked-scope-action"]         ;
export const RISK_LEVELS = ["low", "medium", "high"]         ;
export const REVIEW_LENSES = ["review-risk", "review-resilience", "review-readability", "review-reliability"]         ;
export const RISK_REASON_CODES = ["configuration_change", "empty_content", "executable_change", "executable_mode", "hot_path", "large_change", "non_executable_only", "process_boundary", "process_scan_limit", "service_token", "shell_source"]         ;
export const RISK_SIGNALS = ["auth", "update", "security", "payments", "permissions", "shell_process"]         ;
// “collect” 与 “execute” 是 v2 封套对 next_transition 必填的活动事务的
// 投影：根动作点名转换种类，而不是读作终局 stop。
export const STATUS_ACTIONS = ["start", "recover", "maintainer_action", "select_lineage", "repair_authority", "stop", "collect", "execute"]         ;
export const RECEIPT_STATUSES = ["expected_missing", "present", "publication_pending", "not_applicable"]         ;
...
export const REQUIRED_GATES = Object.freeze(["post-apply", "pre-commit", "pre-push", "pre-pr", "release"]         );
...
export const CAPABILITIES_SCHEMA_IDENTITIES                                                                                                                                                                        = Object.freeze({
...
export const REQUIRED_MANDATORY_FEATURES = Object.freeze(FEATURE_NAMES.filter((name) => !(OPTIONAL_FEATURE_NAMES                     ).includes(name)));
