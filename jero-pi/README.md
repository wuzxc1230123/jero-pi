...
- **进程内评审权威**（`lib/authority/`）：START → 同意 → relay 评审视角 → 冻结/分类 → 有界纠正 → 定向校验 → 确认销毁 → 维护。状态存于 `.git/jero-review/`，随仓库走；actor 产物（模型输出）永远是无信托数据，不能授权任何状态转移。
...
- 技术参考：[docs/jero-reference.md](docs/jero-reference.md)——架构、jero 命名空间、评审权威操作契约、动态 RDD 归属、精益纪律、供应链。
- 保留的历史/契约文档（有意不译，性质说明如下）：
  - `docs/review-integration.md`：**字节钉住的契约工件**（SHA-256 由 `scripts/verify-package-files.mjs` 校验），描述已退役的二进制评审契约，任何改动都会破坏打包门。
  - `docs/native-authority-architecture.md`、`docs/delegated-verification.md`：必需打包资源；前者是评审透镜资产（`assets/agents/review-*.md`）声明的事实来源，被链接解析测试钉住。
  - `docs/skill-style-guide.md`：技能创建/改进技能与 `prompts/skill-creation.md` 共同依赖的规范性风格指南。
  - `docs/readme-reference.md`：上游 gentle-pi v2.7.0 README 原貌快照，仅作历史事实来源。
