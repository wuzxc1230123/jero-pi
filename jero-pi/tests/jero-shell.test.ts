...
import installGentleShell, { buildShellBarModel, changesShortcut, devBinaryCard, fetchCodexUsage, loadFileDiff, shellGitRunner, openInExternalEditor, type GentlePromptEditor } from "../extensions/jero-shell.ts";
import { CHANGE_STATUS } from "../lib/shell/shell-changes.ts";
import { sidebarState, type SidebarRail } from "../lib/shell/shell-sidebar.ts";
import type { ShellBarTheme } from "../lib/shell/shell-bar.ts";
...
test("buildShellBarModel shortens the home directory and hides effort for non-reasoning models", () => {
	...
	(ctx.sessionManager as unknown as { getCwd: () => string }).getCwd = () => "/home/alan/work/jero-pi";
	const footerData = {
		getGitBranch: () => null,
	...
	const built = buildShellBarModel(pi, ctx, footerData, { home: "/home/alan" });
	assert.equal(built.cwd, "~/work/jero-pi");
...
test("gentleShell shows working-tree changes in a widget below the editor and in the bar", async () => {
	...
	const factory = ui.widgets.get("jero-shell-changes") as (tui: unknown, theme: ShellBarTheme) => { render(width: number): string[] };
	const [line] = factory(fakeTui, plainTheme).render(120);
	assert.match(line, /^✎ 1 file · \+10 −0 · lib\/b\.ts +\/jero:changes$/);
	assert.match(renderFooter(ui), /main ±1/);
});

test("gentleShell registers /jero:changes and opens the overlay only when there are changes", async () => {
	...
	const { ctx, ui } = fakeContext();
	await fire(handlers, "session_start", ctx);
	const command = commands.get("jero:changes");
...
test("changes overlay follows registered linked roots on open, refresh and polling and scopes diff git cwd", async () => {
	...
	const open = commands.get("jero:changes")!.handler("", ctx);
	try {
...
test("successful direct tools register whole roots, failures and shell contents do not, and all UI counts agree", async () => {
	...
	const widget = ui.widgets.get("jero-shell-changes") as (host: unknown, theme: unknown) => { render(width: number): string[] };
...
test("session switching drops pending tool and foreign launch events while same-session entries restore", async () => {
	...
	h.pi.events.emit("jero-pi:session-worktree-changed", { sessionId: "shell-session", root: "/foreign" });
...
test("registered canonical root governs real Git discovery, status and diff despite inherited routing", async (t) => {
	...
	const open = h.commands.get("jero:changes")!.handler("", ctx);
	await overlayReady;
	ui.overlayView!.handleInput("\r");
	ui.overlayView!.handleInput("j");
	ui.closeOverlay?.();
...
test("gentleShell watches git in the background so the widget and bar follow external edits", async () => {
	...
	assert.equal(ui.widgets.has("jero-shell-changes"), false);
	await new Promise((resolve) => setTimeout(resolve, 40));
	assert.equal(ui.widgets.has("jero-shell-changes"), true);
	assert.match(renderFooter(ui), /main ±1/);
...
test("gentleShell draws the review preflight message as a Gentle card", () => {
	const { pi } = fakePi();
	gentleShell(pi, {});
	const renderer = renderers.get("jero-pi.review-preflight");
	assert.ok(renderer, "renderer not registered");
	const message = { customType: "jero-pi.review-preflight", content: "Receipt-driven development is enabled.\n\nCall the gentle_review tool." };
