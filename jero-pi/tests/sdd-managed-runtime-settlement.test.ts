...
test("R1 confirms exact canonical paths and commands; data, denial and symlinks grant nothing", async t => {
	...
	assert.match(shown, /pnpm test/); assert.ok(shown.includes(JSON.stringify(target).slice(1, -1))); assert.ok(shown.includes(JSON.stringify(cwd).slice(1, -1)));
