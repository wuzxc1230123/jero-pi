...
import { PROFILES_KIND, PROFILES_VERSION } from "../lib/agents/agent-profiles.ts";
import { NATIVE_REVIEW_ERROR_CODE, NativeReviewCliError, type NativeReviewCli } from "../lib/native/native-review-cli.ts";
import { CandidateViewError, type CandidateViewRegistry } from "../lib/review/review-candidate-view.ts";
import { installPackageAssets } from "../lib/sdd/sdd-preflight.ts";
...
function reviewRepository(t: test.TestContext): ReviewStartRepository {
	const cwd = realpathSync(mkdtempSync(join(tmpdir(), "jero-pi-review-start-")));
	t.after(() => {
...
test("missing package-local binaries give a direct recovery without attributing the cause to lifecycle scripts", async () => {
	...
	assert.match(String(result.next_action), /installed jero-pi package directory/);
...
function routingConsumerFixture(t: test.TestContext, agents = ["worker"]) {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-routing-consumers-"));
...
test("session startup reports invalid project routing without mutating the profile", async (t) => {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-model-routing-startup-"));
	const configHome = join(root, "global");
...
test("agent discovery skips skills directories", async (t) => {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-agents-"));
	t.after(() => rmSync(root, { recursive: true, force: true }));
...
test("managed routing timeout leaves its profile, agent, and manifest unchanged", (t) => {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-managed-routing-timeout-"));
	const agentHome = join(root, "agent-home");
	const previousAgentHome = process.env.GENTLE_PI_AGENT_HOME;
	t.after(() => {
		if (previousAgentHome === undefined) delete process.env.GENTLE_PI_AGENT_HOME;
...
test("a later alias keeps managed-root precedence and manifest ownership", (t) => {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-agent-root-alias-"));
	const agentHome = join(root, "agent-home");
...
test("agent model discovery prioritizes SDD and Judgment Day agents", (t) => {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-model-agents-"));
	t.after(() => rmSync(root, { recursive: true, force: true }));
...
test("discoverable model agents include installed Judgment Day agents", (t) => {
	const root = mkdtempSync(join(tmpdir(), "jero-pi-installed-agents-"));
...
test("guarded command confirmation emits a generic correlated permission lifecycle", async () => {
	...
	const cwd = mkdtempSync(join(tmpdir(), "jero-pi-permission-request-"));
	try {
		const ctx = {
			cwd,
			hasUI: true,
...
test("concurrent guarded confirmations coalesce the Herdr lifecycle per extension instance", async () => {
	...
	const cwd = mkdtempSync(join(tmpdir(), "jero-pi-permission-concurrent-"));
	try {
...
test("RPIV questionnaire blockers emit only a private, balanced Herdr projection", () => {
	...
	assert.equal(eventHandlers.has("jero-pi:ask-user-choice:blocked"), true);
...
test("closed choice blockers retain the visible choice label through guarded-confirmation overlap", async () => {
	...
	const pi = {
		...
		events: {
			emit(channel: string, data: unknown) {
				if (channel === "herdr:blocked") herdrEvents.push(data as HerdrBlockedEvent);
				if (channel === "jero-pi:ask-user-choice:blocked") choiceEvents.push(data as { active: boolean });
	...
	pi.events.emit("jero-pi:ask-user-choice:blocked", { active: true });
	...
	pi.events.emit("jero-pi:ask-user-choice:blocked", { active: false });
...
test("permission lifecycle is inactive for unguarded and headless commands", async () => {
	...
	const toolCall = handlers.get("tool_call");
	assert.equal(typeof toolCall, "function");
	const cwd = mkdtempSync(join(tmpdir(), "jero-pi-permission-headless-"));
	try {
		const confirm = async () => {
			confirmations += 1;
			return true;
...
test("bash tool_call confirms a late guarded npm publish and denies on non-approval", async () => {
	...
	const configHome = mkdtempSync(join(tmpdir(), "jero-pi-guard-confirm-"));
	const ctx = {
		cwd: process.cwd(),
		hasUI: true,
		ui: {
...
test("bash tool_call confirms every compound action and centers a long git -C push", async () => {
	...
	const configHome = mkdtempSync(join(tmpdir(), "jero-pi-guard-compound-"));
	const previousConfigHome = process.env.GENTLE_PI_CONFIG_HOME;
	process.env.GENTLE_PI_CONFIG_HOME = configHome;
	try {
...
// /jero:profiles reopens its panel after every action, so a test that applies
