...
test("a transport refusal names the runnable wrapper continuation, never a raw CLI command as the only exit", async (t) => {
	...
	// surface: the jero_review / jero_review_capture wrapper tools. The
	...
	assert.match(captureNextAction, /jero_review \{"operation":"inspect"\}/, "the capture refusal re-enters negotiated STATUS through the wrapper controller");
	assert.match(captureNextAction, /jero_review_capture/, "the capture refusal names the wrapper resubmission, not a CLI command");
	...
	assert.deepEqual(capture.wrapper_continuation, { tool: "jero_review", operation: "inspect", then: "jero_review_capture" });
	...
	assert.match(statusNextAction, /jero_review \{"operation":"inspect"\}/, "the controller refusal re-enters negotiated STATUS through the wrapper controller");
	...
	assert.deepEqual(status.wrapper_continuation, { tool: "jero_review", operation: "inspect" });
});
