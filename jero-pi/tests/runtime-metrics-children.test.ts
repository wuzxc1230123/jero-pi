...
test("packaged sdd-proposal is encoded only as canonical sdd-propose", () => {
	...
	assert.ok(!JSON.stringify(snapshot.responses).includes("sdd-proposal"));
});
...
test("registered child launch identity survives catalog state and missing response evidence inherits selection", () => {
	...
	assert.deepEqual(snapshot.launches[0], { evidence: "launch_configuration", agentClass: "sdd-explore",
		selectedProvider: "openai-codex", selectedModelId: "gpt-5.6-terra", selectedEffort: "high", launches: 1 });
