...
import { pendingReviewMutation, REVIEW_REMINDER_RECEIPT } from "../lib/review/review-reminder-receipt.ts";
import { SESSION_WORKTREE_ENTRY, SESSION_WORKTREE_CHANGED } from "../lib/core/session-worktree-registry.ts";
import test, { after, mock } from "node:test";
import type { ExtensionAPI, ExtensionContext } from "@earendil-works/pi-coding-agent";
import { visibleWidth, type TuiMouseEvent } from "@earendil-works/pi-tui";
import gentleAgents, { agentRuntimePaths, agentsCollapseKey, agentsEnabled, agentsStopKey, agentsViewKey, answerThroughUi, completionText, legacySubagentsInstalled, type AgentsDeps } from "../extensions/jero-agents.ts";
import { historyDir, loadHistory, saveTask } from "../lib/agents/agents-history.ts";
import { STALE_COMPLETION_MS } from "../lib/agents/agents-completion-delivery.ts";
import { emptyThread, TASK_EVENT, TASK_STATUS, TaskStore, type TaskRecord } from "../lib/agents/agents-protocol.ts";
import { NativePointerScope } from "../lib/native/native-pointer-region.ts";
import { PresenceCursor, PresencePublisher, listPresence, readActivity } from "../lib/core/orchestrator-presence.ts";
...
import { CHILD_METRICS_EVENT } from "../lib/metrics/runtime-metrics-children.ts";
import { renderSddPreflightPrompt } from "../lib/sdd/sdd-preflight.ts";
...
for (const boundary of ["allowed", "env", "session", "replacement", "bus-throws", "no-spawn", "long-running"] as const) {
	test(`local child composition through real extensions and RPC runner: ${boundary}`, async (t) => {
		...
		if (admitted) {
			...
			const event = events[0].data as import("../lib/metrics/runtime-metrics-children.ts").ChildMetricsEvent;
...
test("while pi-subagents-j0k3r is still installed the tools stay unregistered and the user is told how to switch", async () => {
	...
	writeFileSync(join(legacyHome, ".pi", "agent", "settings.json"), JSON.stringify({ packages: ["npm:pi-subagents-j0k3r", "../../work/jero-pi"] }));
