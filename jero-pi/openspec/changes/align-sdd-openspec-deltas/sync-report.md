...
- Added `sdd-sync` to SDD agent ordering in `extensions/jero-ai.ts`.
...
- Updated `/jero:status` drift detection so missing packaged SDD assets count as stale.
...
- Existing installed `.pi` assets remain stale until the user intentionally refreshes with `/jero:install-sdd --force`.
