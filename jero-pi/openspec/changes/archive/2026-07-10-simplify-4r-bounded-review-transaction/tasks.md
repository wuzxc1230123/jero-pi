...
- [x] 1.2 GREEN/TRIANGULATE/REFACTOR — create `lib/review/review-snapshot.ts` using isolated Git objects/index; reject unresolved or unsupported projections.
...
- [x] 2.2 GREEN — create `lib/review/review-transaction.ts` with const-derived flat V1 schemas, atomic revisions/HEAD, journal, frozen rows, receipts, budgets, and deterministic claims.
...
- [x] 3.2 GREEN/TRIANGULATE/REFACTOR — create `lib/review/review-policy-ordinary.ts`; keep claims immutable, resolutions separate, counters monotonic, and failures terminal.
...
- [x] 4.2 GREEN/TRIANGULATE/REFACTOR — create `lib/review/review-policy-judgment-day.ts` with disjoint phases, immutable mode/budget, and no third-round edge.

## Phase 5: Operations and Gates

- [x] 5.1 RED — extend `tests/review-triggers.test.ts`, `tests/review-gate.test.ts`, and `tests/jero-ai.test.ts` for start-only routing, exact targets, fail-closed forms, replay, zero actors, and command safety.
- [x] 5.2 GREEN/TRIANGULATE/REFACTOR — update `lib/review/review-triggers.ts`, `lib/review/review-transaction.ts`, and `extensions/jero-ai.ts` with typed controller operations, gate unions, receipt validation, and scope-child results.
...
- [x] 6.3 TRIANGULATE/REFACTOR — update `lib/sdd/sdd-preflight.ts` and `scripts/verify-package-files.mjs`; migrate only untouched package-owned contracts and preserve user routing/overrides.
