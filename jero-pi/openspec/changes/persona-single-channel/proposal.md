...
- Documented cross-tool ownership contract: jero-pi owns Pi-session identity/persona.
...
|------|--------|-------------|
| `extensions/jero-ai.ts` (:148-201) | Modified | Canonical identity/persona/language block; single constant source |
...
| cross-tool contract doc | New | jero-pi ownership; gentle-ai follow-up expectation |

### Measured before/after (per parent session)

| Channel | Before | After | Note |
|---------|--------|-------|------|
| jero-pi wrapper | ~1.9 KB | ~1.9 KB | canonical home (union) |
| `orchestrator.md` | 22,626 B | ~22.6 KB − dup | Identity (~0.9 KB) + Lang Boundary (~2.4 KB) copies → ~0.2 KB pointer |
| jero-pi injection dedup | — | **≈ −3.1 KB** | exact delta frozen in apply RED test (chars/4 + byte assert) |
...
| Cross-tool persona drift after gentle-ai follow-up | Low | Ownership contract doc states jero-pi is canonical for Pi sessions |
...
- [ ] Identity/persona/language stated once in jero-pi's injection; other location is a pointer.
- [ ] Frozen-fixture test proves union (zero normative rules lost) and asserts the measured byte reduction.
- [ ] `pnpm test` green.
- [ ] Cross-tool ownership contract documented (jero-pi owns Pi-session identity; gentle-ai follow-up recorded).
